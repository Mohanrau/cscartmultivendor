...
class SchemesManager
{
                ...
                foreach ($block_scheme['templates'] as $path => $template) {
                    foreach ($theme_paths as $path_info) {
                        ...
                        if (empty($template['name'])) {
                            $template['name'] = self::generateTemplateName($path, $path_info[Themes::PATH_ABSOLUTE]);
                        }
    ...
    private static function _getValue($item)
    {
        ...
        if ($single_template) {
            return array(
                strval($item) => array(
                    'name' => self::generateTemplateName($item, $theme_paths[$single_template['theme']][Themes::PATH_ABSOLUTE])
                )
            );
        }
