...
class Exim
{
    ...
    public function importSnippet(array $data, $lang_code = DESCR_SL)
    {
        ...
        if ($result->isSuccess()) {
            foreach ($descriptions as $lang_code => $items) {
                $this->repository->updateDescription($snippet->getId(), $items, $lang_code);
            }
    ...
    public function importTableColumn(array $data, $lang_code = DESCR_SL)
    {
        ...
        if ($result->isSuccess()) {
            foreach ($descriptions as $lang_code => $items) {
                $this->repository->updateDescription($column->getId(), $items, $lang_code);
            }
        }
