    ...
    public function __construct(array $config)
    {
        ...
        if (!isset($config['attributes'])) {
            ...
            if ($reflexion_class->implementsInterface('\Tygh\Template\IActiveVariable')) {
                $config['attributes'] = call_user_func(array($this->getClass(), 'attributes'));
            } else {
                ...
                foreach ($properties as $property) {
                    if (!$property->isStatic()) {
                        $config['attributes'][] = $property->getName();
                    }
                }
                ...
                foreach ($methods as $method) {
                    if (!$method->isStatic()) {
                        ...
                        if (strpos($method_name, 'get') === 0) {
                            ...
                            $config['attributes'][] = $this->underscore($method_name);
                        }
                    }
                }
            }
        } elseif ($config['attributes'] instanceof \Closure) {
            $config['attributes'] = call_user_func($config['attributes']);
        }

        $this->attributes = $this->convertAttributes((array) $config['attributes']);
    }
