    ...
    public function testImportSnippet($snippet_data, $snippet)
    {
        ...
        if (!empty($snippet_data['extra']['table_columns'])) {
            ...
            foreach ($snippet_data['extra']['table_columns'] as $key => $item) {
                $this->column_repository->expects($this->at($key + 1))->method('findBySnippetAndCode')->with($snippet_data['type'], $snippet_data['code'], $item['code']);

                if (strpos($item['code'], 'exist') !== false) {
                    $this->column_service->expects($this->at($key))->method('updateColumn');
                } else {
                    $this->column_service->expects($this->at($key))->method('createColumn');
                }
            }
        }
