...
function fn_data_feeds_get_product_features($product_id, $field, $lang_code, $e_lang_code, $v_feature_id)
{
    ...
    $result = false;
    if (!empty($feature_id)) {
        $feature_values = db_get_array('SELECT var_descr.variant, feature_val.value, feature_val.value_int FROM ?:product_feature_variant_descriptions AS var_descr RIGHT JOIN ?:product_features_values AS feature_val ON (feature_val.variant_id = var_descr.variant_id AND feature_val.lang_code = var_descr.lang_code) WHERE feature_val.feature_id = ?i AND feature_val.product_id = ?i AND feature_val.lang_code = ?s GROUP BY feature_val.variant_id', $feature_id, $product_id, $lang_code);
        $variants = array();
