...
{
    $layout_data = db_get_row("SELECT cols, options, active FROM ?:exim_layouts WHERE name = 'general_data_feeds' AND pattern_id = 'data_feeds'");
    $layout_data['name'] = 'google_export';
    $layout_data['pattern_id'] = 'data_feeds';

    $layout_id = db_get_field("SELECT layout_id FROM ?:exim_layouts WHERE name = 'general_data_feeds' AND pattern_id = 'data_feeds'");
    ...
    if (!empty($parent_feature_id)) {
        foreach ($fields as $k_field => $field) {
            $feature_id = db_get_field("SELECT ?:product_features_descriptions.feature_id FROM ?:product_features_descriptions LEFT JOIN ?:product_features ON ?:product_features_descriptions.feature_id = ?:product_features.feature_id WHERE ?:product_features_descriptions.description = ?s AND ?:product_features_descriptions.lang_code = ?s AND ?:product_features.parent_id = ?i", $field['field'], DEFAULT_LANGUAGE, $parent_feature_id);
...
function fn_export_get_options_product_google_export(&$result, &$export_fields, $multi_lang)
{
    $f_option = false;
    $data_products = array();
        ...
        foreach ($multi_lang as $lang_code) {
            ...
            if (!empty($options)) {
                ...
                foreach ($options as $name_option => $option) {
                    ...
                    if (!empty($_options)) {
                        fn_google_export_generate_product_options($combinations, $combination, $c_id, $_options, $export_options);
                    } else {
                ...
                foreach ($combinations as $combination) {
                    ...
                    $cart_id = fn_generate_cart_id($d_product[$lang_code]['Product id'], $product_options);

                    $combination['item_group_id'] = $d_product[$lang_code]['Product id'];
                    $combination['Product id'] = $cart_id;
                    $data_products[][$lang_code] = array_merge($d_product[$lang_code], $combination);
...
function fn_google_get_product_options($product_id, $field, $lang_code, $p_lang_code = "")
{
    ...
    $f_options = db_get_row('SELECT opt_des.option_id, opt_des.option_name, opt.google_export_name_option, opt.product_id FROM ?:product_options_descriptions as opt_des LEFT JOIN ?:product_options as opt ON opt_des.option_id = opt.option_id WHERE lang_code = ?s AND google_export_name_option = ?s AND product_id = ?i', $lang_code, $field, $product_id);

    if (empty($f_options)) {
        $f_options = db_get_row('SELECT opt_des.option_id, opt_des.option_name, opt.google_export_name_option, opt.product_id FROM ?:product_options_descriptions as opt_des LEFT JOIN ?:product_options as opt ON opt_des.option_id = opt.option_id WHERE lang_code = ?s AND google_export_name_option = ?s AND product_id = 0', $lang_code, $field);

        if (!empty($f_options['option_id'])) {
            $option_id = db_get_field('SELECT option_id FROM ?:product_global_option_links WHERE option_id = ?i AND product_id = ?i', $f_options['option_id'], $product_id);
            if (empty($option_id)) {
                $f_options = array();
            }
        }
    ...
    $result = false;
    if (!empty($f_options)) {
        $option_variants = db_get_array('SELECT var_descr.variant_name, option_var.option_id, option_var.variant_id FROM ?:product_option_variants_descriptions AS var_descr RIGHT JOIN ?:product_option_variants AS option_var ON (option_var.variant_id = var_descr.variant_id) WHERE option_var.option_id = ?i AND var_descr.lang_code = ?s GROUP BY var_descr.variant_id', $f_options['option_id'], $lang_code);
