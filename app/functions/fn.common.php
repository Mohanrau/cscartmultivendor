...
/*
 * Function defines and assigns pages features
 *
 * @param array  $request params to generate pagination from
...
function fn_get_paginated_product_features($request, $auth, $product_data = array(), $lang_code = DESCR_SL)
{
    ...
    $views = fn_get_views('lv_detail_features');
    if (!empty($views)) {
        $_view = reset($views);
        $_params['view_id'] = $_view['view_id'];
    }
