...
function fn_get_report_statistics(&$table)
{
    ...
    foreach ($table['elements'] as $element) {
        ...
        if ($table['display'] == 'order_amount') {
            ...
            $data[$a] = db_get_hash_array("SELECT $fields as total $add_field FROM $tables WHERE $element[request] AND $time_condition $order_ids ?p $group_condition", 'interval', $product_cost);
