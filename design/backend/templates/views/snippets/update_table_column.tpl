        ...
        <div class="buttons-container" id="content_tab_table_column_buttons_{$id}">
            {if $id && $column->isModified()}
                ...
                {capture name="tools_list"}
                    <li>{btn type="text" href="snippets.restore_table_column?column_id=$id&return_url=$r_url" class="cm-confirm cm-ajax" data=["data-ca-target-id" => "content_tab_table_column_{$id},content_tab_table_column_buttons_{$id}", "data-ca-confirm-text" => "{__("text_restore_question")}"] text=__("restore") method="POST"}</li>
                {/capture}
