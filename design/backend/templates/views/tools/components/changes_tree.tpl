...
<tr {if $item.level % 2}class="multiple-table-row"{/if}>
    ...
    {strip}
        <span style="padding-left: {$shift}px;">
            {if $item.content}
                {if $show_all}
                    <span title="{__("expand_sublist_of_items")}" id="on_changes_{$item_id}" class="hand cm-combination {if $expand_all}hidden{/if}"><span class="icon-caret-right"></span></span>
                {else}
                ...
                {/if}
                <span alt="{__("collapse_sublist_of_items")}" title="{__("collapse_sublist_of_items")}" id="off_changes_{$item_id}" class="hand cm-combination{if !$expand_all || !$show_all} hidden{/if}"><span class="icon-caret-down"></span></span>
            {else}
                &nbsp;
            {/if}
